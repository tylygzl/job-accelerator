...
- 2026-08-05 C0 正常真实未读回归通过：2 条真实未读按 2 个会话入队，三次扫描不重复，页面刷新后队列恢复；用户主动处理后进入正确会话，未再出现会话不一致。随后本地回复接口连接失败，插件保守转为 `needs_user`，未填草稿、未自动发送且未影响海投；本次只证明会话定位与失败兜底，不声称后端草稿生成成功。
- 2026-08-05 Manifest V3 service worker 强制终止并重启后，待回复队列和徽标均恢复为 2，状态保持，未自动跳转、填草稿或发送消息。
...
- 阶段 B/C0 正常真实未读和 service worker 强制重启恢复已通过；B5-5 海投运行期间新出现真实未读仍为唯一延期验收。
- 正式扩大分发前关闭开发测试模式；朋友包仍早于 C0 修复，只有用户明确授权后才重打并执行 release 内容审核。
